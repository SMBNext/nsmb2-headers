...
void LoaderMain()
{
    ...
    Handle processHandle = getCurrentProcessHandle();

    // Reprotect custom code as RWX
    res = svcControlProcessMemory(processHandle, address, address, neededMemory, 6, 7);
    if (res < 0)
        svcBreak(1);

    // Reprotect rodata as RW
    res = svcControlProcessMemory(processHandle, 0x0056B000, 0x0056B000, 0x70000, 6, 3);
    if (res < 0)
        svcBreak(1);
}
